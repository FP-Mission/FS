...
namespace App{
  ...
  IMU_ST_ANGLES_DATA& Icm::getAngles()
  {
  ...
  void Icm::writeOneByte(char reg, char val){
  ...
  void Icm::imuDataGet(IMU_ST_ANGLES_DATA *pstAngles, 
...
void Icm::imuAHRSupdate(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz) 
{
  float norm;
...
void Icm::icm20948init(void)
{

  /* user bank 0 register */
  writeOneByte( REG_ADD_REG_BANK_SEL, REG_VAL_REG_BANK_0);
  writeOneByte( REG_ADD_PWR_MIGMT_1,  REG_VAL_ALL_RGE_RESET);
  delay(10);
  writeOneByte( REG_ADD_PWR_MIGMT_1,  REG_VAL_RUN_MODE);  

  /* user bank 2 register */
  writeOneByte( REG_ADD_REG_BANK_SEL, REG_VAL_REG_BANK_2);
  writeOneByte( REG_ADD_GYRO_SMPLRT_DIV, 0x07);
  writeOneByte( REG_ADD_GYRO_CONFIG_1,   
                  REG_VAL_BIT_GYRO_DLPCFG_6 | REG_VAL_BIT_GYRO_FS_1000DPS | REG_VAL_BIT_GYRO_DLPF);
  writeOneByte( REG_ADD_ACCEL_SMPLRT_DIV_2,  0x07);
  writeOneByte( REG_ADD_ACCEL_CONFIG,
...
bool Icm::icm20948Check(void)
{
    bool bRet = false;
    if(REG_VAL_WIA == readOneByte( REG_ADD_WIA))
    {
        bRet = true;
    }
    return bRet;
}
void Icm::icm20948GyroRead(short int* ps16X, short int* ps16Y, short int* ps16Z)
{
   unsigned char  u8Buf[6];
    ...
    u8Buf[0]=readOneByte(REG_ADD_GYRO_XOUT_L); 
    u8Buf[1]=readOneByte(REG_ADD_GYRO_XOUT_H);
    s16Buf[0]=  (u8Buf[1]<<8)|u8Buf[0];

    u8Buf[0]=readOneByte(REG_ADD_GYRO_YOUT_L); 
    u8Buf[1]=readOneByte(REG_ADD_GYRO_YOUT_H);
    s16Buf[1]=  (u8Buf[1]<<8)|u8Buf[0];

    u8Buf[0]=readOneByte(REG_ADD_GYRO_ZOUT_L); 
    u8Buf[1]=readOneByte(REG_ADD_GYRO_ZOUT_H);
...
void Icm::icm20948AccelRead(short int* ps16X, short int* ps16Y, short int* ps16Z)
{
    ...
    u8Buf[0]=readOneByte(REG_ADD_ACCEL_XOUT_L); 
    u8Buf[1]=readOneByte(REG_ADD_ACCEL_XOUT_H);
    s16Buf[0]=  (u8Buf[1]<<8)|u8Buf[0];

    u8Buf[0]=readOneByte(REG_ADD_ACCEL_YOUT_L); 
    u8Buf[1]=readOneByte(REG_ADD_ACCEL_YOUT_H);
    s16Buf[1]=  (u8Buf[1]<<8)|u8Buf[0];

    u8Buf[0]=readOneByte(REG_ADD_ACCEL_ZOUT_L); 
    u8Buf[1]=readOneByte(REG_ADD_ACCEL_ZOUT_H);
    s16Buf[2]=  (u8Buf[1]<<8)|u8Buf[0];
...
void Icm::icm20948MagRead(short int* ps16X, short int* ps16Y, short int* ps16Z)
{
...
void Icm::icm20948ReadSecondary(unsigned char  u8I2CAddr, unsigned char  u8RegAddr, unsigned char  u8Len, unsigned char  *pu8data)
{
    unsigned char  i;
    unsigned char  u8Temp;

    writeOneByte( REG_ADD_REG_BANK_SEL,  REG_VAL_REG_BANK_3); //swtich bank3
    writeOneByte( REG_ADD_I2C_SLV0_ADDR, u8I2CAddr);
    writeOneByte( REG_ADD_I2C_SLV0_REG,  u8RegAddr);
    writeOneByte( REG_ADD_I2C_SLV0_CTRL, REG_VAL_BIT_SLV0_EN|u8Len);

    writeOneByte( REG_ADD_REG_BANK_SEL, REG_VAL_REG_BANK_0); //swtich bank0
    
    u8Temp = readOneByte(REG_ADD_USER_CTRL);
    u8Temp |= REG_VAL_BIT_I2C_MST_EN;
    writeOneByte( REG_ADD_USER_CTRL, u8Temp);
    delay(5);
    u8Temp &= ~REG_VAL_BIT_I2C_MST_EN;
    writeOneByte( REG_ADD_USER_CTRL, u8Temp);
    ...
    for(i=0; i<u8Len; i++)
    {
        *(pu8data+i) = readOneByte( REG_ADD_EXT_SENS_DATA_00+i);
        
    }
    writeOneByte( REG_ADD_REG_BANK_SEL, REG_VAL_REG_BANK_3); //swtich bank3
    
    u8Temp = readOneByte(REG_ADD_I2C_SLV0_CTRL);
    u8Temp &= ~((REG_VAL_BIT_I2C_MST_EN)&(REG_VAL_BIT_MASK_LEN));
    writeOneByte( REG_ADD_I2C_SLV0_CTRL,  u8Temp);
    
    writeOneByte( REG_ADD_REG_BANK_SEL, REG_VAL_REG_BANK_0); //swtich bank0

}

void Icm::icm20948WriteSecondary(unsigned char  u8I2CAddr, unsigned char  u8RegAddr, unsigned char  u8data)
{
  unsigned char  u8Temp;
  writeOneByte( REG_ADD_REG_BANK_SEL,  REG_VAL_REG_BANK_3); //swtich bank3
  writeOneByte( REG_ADD_I2C_SLV1_ADDR, u8I2CAddr);
  writeOneByte( REG_ADD_I2C_SLV1_REG,  u8RegAddr);
  writeOneByte( REG_ADD_I2C_SLV1_DO,   u8data);
  writeOneByte( REG_ADD_I2C_SLV1_CTRL, REG_VAL_BIT_SLV0_EN|1);
...
void Icm::icm20948CalAvgValue(unsigned char  *pIndex, short int *pAvgBuffer, short int InVal, int *pOutVal)
{ 
  unsigned char  i;
...
void Icm::icm20948GyroOffset(void)
{
  unsigned char  i;
...
bool Icm::icm20948MagCheck(void)
{
    bool bRet = false;
